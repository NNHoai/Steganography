$('button.encodebtn, button.decode').click(function(event) {
  ...
  function loadaudio() {
    var file = document.getElementById('audio_input');
    audio.src = "../Audio/"+file.value.replace(/.*[\/\\]/, '');
    $("#audio-element").fadeIn();
    
    const inputNode = document.getElementById('audio_input')
    const selectedFile = inputNode.files[0];
    const byteFile = getAsByteArray(selectedFile);
    console.log(byteFile);
    const buffer = new ArrayBuffer(inputNode);
  ...
  function previewEncodeImage() {
    ...
    previewImage(file, ".form canvas", function() {
      $(".form .original").fadeIn();
      $(".form").fadeIn();
    });
  }
